...
## Back to qlearning_path_finding
client = mqtt.Client()
root = Tk()
DEFAULT_SIZE_ROW = 5
DEFAULT_SIZE_COLUMN = 5
##DEFAULT_SIZE_ROW = 8
##DEFAULT_SIZE_COLUMN = 8
MAX_SIZE = 10
GRID_SIZE = 30
PADDING = 5
...
current_color = NOT_USE  # Initialize current_color variable
...
class QLearningTable:
    ...
    def choose_action(self, current_state, valid_actions):
        ...
        if np.random.uniform() < self.epsilon:
            # choose best action
            state_action = self.q_table.loc[np.array_str(current_state), :]
            temp = np.array(state_action.values)
    ...
    def learn(self, current_state, action, reward, next_state, game_over):
        ...
        q_predict = self.q_table.loc[np.array_str(current_state), action]
        if not game_over:
            q_target = reward + self.gamma * self.q_table.loc[np.array_str(next_state),
                                             :].max()  # next state is not terminal
        else:
            q_target = reward  # next state is terminal
        self.q_table.loc[np.array_str(current_state), action] += self.lr * (q_target - q_predict)  # update
...
def onObjectClick(event):
    ...
    if (row < row_num and col < col_num):
        ...
        valid_value = np.array([EMPTY, START, TARGET, OBSTACLE], dtype=float)
...
def trainDQN():
    ...
    ##set_trellis_color()  # Update the colors on the NeoTrellis to match the GUI grid
    pass
...
if __name__ == "__main__":
    ...
    ##Button(frame4, text="Reset", command=resetEV3Query).pack(side=LEFT)
    ##btnSendQuery = Button(frame4, text="Start run EV3", command=sendEV3Query, state=DISABLED)
    ##btnSendQuery = Button(frame4, text="Epsilon", command=sendEV3Query, state=DISABLED)
    ...
    storage_value = np.array(storage_value, dtype=float).reshape(10, 10)
    canvas.pack()
